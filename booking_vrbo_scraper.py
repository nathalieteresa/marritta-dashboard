            ...
            for i in range(min(anchors.count(), 250)):
                try:
                    ...
                    if not href or href in seen:
                        continue
                    href_l = href.lower()
                    if "vrbo.com" not in href_l:
                        continue
                    # VRBO changes property URL formats often. Keep broad, but avoid utility/account links.
                    property_like = any(x in href_l for x in ["/vacation-rental/", "/p/", "/ha/", "rooms-and-rates", "pdp"])
                    blocked_like = any(x in href_l for x in ["/search", "/login", "/trips", "/help", "privacy", "terms"])
                    if not property_like or blocked_like:
                        continue
                    card = anchors.nth(i).evaluate("el => el.closest('[data-stid], article, li, div')?.innerText || el.innerText || ''") or ""
                    seen.add(href)
                    candidates.append({"url": href.split("?")[0], "card_text": _norm(card)})
                except Exception:
                    pass
            # Fallback: if VRBO exposes prices on search cards but hides detail links/selectors,
            # keep visible priced cards so the dashboard is not empty.
            if not candidates:
                body_text = _norm(page.locator("body").inner_text(timeout=5000))
                chunks = re.split(r"(?=\$\s?[1-9][0-9,]{2,})", body_text)
                for idx, chunk in enumerate(chunks[:20]):
                    price = _price_from_text(chunk)
                    if price and any(word in chunk.lower() for word in ["sunny", "beach", "condo", "apartment", "bedroom"]):
                        specs = _parse_specs(chunk)
                        ok, reason = _matches_core_specs(specs, chunk)
                        if ok:
                            rows.append(_make_row(f"VRBO visible result {idx+1}", search_url, chunk, price, "VRBO", specs, reason))
        except Exception:
            pass
        finally:
            try: page.close()
...
def get_booking_prices(checkin, checkout, max_detail_pages=12, max_seconds=180, use_cache=True):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if cards.count() == 0:
                # Booking.com sometimes changes selectors or serves a simplified layout.
                # Fallback to divs/articles that visibly contain prices.
                cards = page.locator("article:has-text('$'), div:has-text('$')")
