...
@st.cache_data(ttl=60 * 60 * 3, show_spinner="Analizando competidores en Airbnb... puede tardar varios minutos la primera vez.")
def get_cached_airbnb_prices(checkin: str, checkout: str):
    # Airbnb-only mode: give the scraper the full time budget instead of splitting time with Booking/VRBO.
    return get_airbnb_prices(checkin, checkout, max_detail_pages=24, max_seconds=240, use_cache=True)
...
def tr(text):
    ...
    manual = {
        ...
        "AI-powered pricing recommendations using Airbnb competitor data": "Recomendaciones de precios con datos de competidores en Airbnb impulsadas por IA",
...
st.markdown(
    ...
    <div class="hero-card">
        ...
        <div class="hero-subtitle">{tr("AI-powered pricing recommendations using Airbnb competitor data")}</div>
    </div>
    """,
    unsafe_allow_html=True
)
...
if st.button(f"🔍 {tr('Analyze Market')}"):
    ...
    if len(listings) > 0:
        ...
        all_competitor_df = competitor_df.copy()
        ...
        with st.expander(f"🏠 {tr('Competitor overview')}"):
            ...
            m4.metric(
                ...
                help=tr("This is the total number of Airbnb competitor listings found for the selected dates.")
            )
...
with st.sidebar:
    ...
    st.markdown(f'<div class="sidebar-status">✅ Airbnb scan active</div>', unsafe_allow_html=True)
