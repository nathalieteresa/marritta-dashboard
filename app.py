...
try:
    ...
    BOOKING_VRBO_IMPORT_ERROR = str(e)
else:
    BOOKING_VRBO_IMPORT_ERROR = None

# Keep the scrape fast and balanced: only top 4 results per platform.
TOP_RESULTS_PER_SOURCE = 4
AIRBNB_TIME_LIMIT_SECONDS = 60
BOOKING_TIME_LIMIT_SECONDS = 75
VRBO_TIME_LIMIT_SECONDS = 75

@st.cache_data(ttl=60 * 60 * 3, show_spinner="Analizando top 4 de Airbnb...")
def get_cached_airbnb_prices(checkin: str, checkout: str):
    return get_airbnb_prices(
        checkin,
        checkout,
        max_detail_pages=TOP_RESULTS_PER_SOURCE,
        max_seconds=AIRBNB_TIME_LIMIT_SECONDS,
        use_cache=True,
    )

@st.cache_data(ttl=60 * 60 * 3, show_spinner="Analizando top 4 de Booking.com...")
def get_cached_booking_prices(checkin: str, checkout: str):
    if get_booking_prices is None:
        return []
    return get_booking_prices(
        checkin,
        checkout,
        max_detail_pages=TOP_RESULTS_PER_SOURCE,
        max_seconds=BOOKING_TIME_LIMIT_SECONDS,
        use_cache=True,
    )

@st.cache_data(ttl=60 * 60 * 3, show_spinner="Analizando top 4 de VRBO...")
def get_cached_vrbo_prices(checkin: str, checkout: str):
    if get_vrbo_prices is None:
        return []
    return get_vrbo_prices(
        checkin,
        checkout,
        max_detail_pages=TOP_RESULTS_PER_SOURCE,
        max_seconds=VRBO_TIME_LIMIT_SECONDS,
        use_cache=True,
    )
...
if st.button(f"🔍 {tr('Analyze Market')}"):
    ...
    with st.spinner("Analizando top 4 de Airbnb, top 4 de Booking.com y top 4 de VRBO..."):
    ...
    st.caption(
        f"Sources found: Airbnb {len(listings)} · Booking.com {len(booking_listings)} · VRBO {len(vrbo_listings)} "
        f"| Limit: top {TOP_RESULTS_PER_SOURCE} per source"
    )
    if BOOKING_VRBO_IMPORT_ERROR:
        st.warning(f"Booking/VRBO scraper could not load: {BOOKING_VRBO_IMPORT_ERROR}")
